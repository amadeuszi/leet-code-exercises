...
function deserialize(data: string): TreeNode | null {
    const array: Array<number | null> = data.split(",")
        .map((value) => (value === "null" || value === "" ? null : Number(value))) as Array<number | null>

    const dfs = (array: Array<number | null>, index: number, node: TreeNode | null) => {
        if (!node) {
            return
        }
        ...
        if (leftValue !== null && leftValue !== undefined) {
            ...
            dfs(array, leftIndex, node.left)
        }
        if (rightValue !== null && leftValue !== undefined) {
            node.right = new TreeNode(rightValue)
            dfs(array, rightIndex, node.right)
        }
    }
...
const twoPowered = (power: number): number => {
    return (1 << power);
}
...
function serialize(root: TreeNode | null): string {
    const h = height(root)
    const maxNodes = twoPowered(h) - 1
    const serialized = Array(maxNodes).fill(null)

    const dfs = (node: TreeNode | null, index: number, result: number[]): number[] => {
        if (!node) {
            return result
        }

        result[index] = node.val
        dfs(node.left, 2 * index + 1, result)
        dfs(node.right, 2 * index + 2, result)

        return result
    }

    return dfs(root, 0, serialized).join(",")
};

// const result = deserialize("1,2,3,null,null,4,5")
// console.log(result)
...
const result2 = deserialize(serialize(deserialize(root)))
console.log(result2)
